...
POLICY_PROMPT_VERSION = "gemini_policy_prompt_v28"
PROMPT_POLICY_LOCK_VERSION = "gemini_policy_lock_v2"
...
PROMPT_LOCKED_JUDGMENT = (
    "== 判断基準 ==\n"
    "1. まず marks_top5 と predictor_horse_probs を見て、量化モデルが高く評価している前5位の馬群を把握すること\n"
    "2. 量化モデルの印をそのまま写す必要はないが、前5位の馬は本命・対抗・相手の中心候補として必ず重点的に扱うこと\n"
    "3. horse_summary を使って、前5位の馬たちの支持の厚さとブレを確認すること\n"
    "4. 量化モデルの順位は top3 確率ベースの評価であり、1着固定の保証ではないことを前提に読むこと\n"
    "5. predictor_horse_probs と consensus_top は、前5位の中でどの馬を上位印に寄せるかを調整する補助情報として使うこと\n"
    "6. 本命や印は前5位の中で自主的に組み替えてよく、odds・支持の厚さ・券種ごとの命中率を合わせて買い方を決めること\n"
    "7. profiles の個性は『どの観点が今回効いていそうか』を説明するために使い、量化前5位から大きく外す理由にしてはいけない\n"
    "8. candidate_tickets の p_hit / quant_support_score / score を見て、量化前5位を軸にしつつ人間らしく命中しやすい買い方を優先すること\n"
...
PROMPT_LOCKED_QUANT_RULES = (
    ...
    "- model_summary.primary_predictor は参考軸の一つだが絶対基準ではなく、本命や印は量化前5位の中で組み替えてよい\n"
    "- まず量化前5位から本命候補群を作り、その後で candidate_tickets の中から買い方と金額を整えること\n"
