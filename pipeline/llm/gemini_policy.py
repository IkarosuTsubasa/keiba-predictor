...
POLICY_PROMPT_VERSION = "gemini_policy_prompt_v29"
PROMPT_POLICY_LOCK_VERSION = "gemini_policy_lock_v3"
_MODULE_DIR = Path(__file__).resolve().parent
_PIPELINE_DIR = _MODULE_DIR.parent
DEFAULT_CACHE_DIR = _PIPELINE_DIR / "data" / "policy_cache_gemini"

# Prompt policy lock:
# The following prompt blocks encode user-approved strategy and must not be
# changed unless the user explicitly asks to change the policy itself.
PROMPT_LOCKED_INTRO = (
...
PROMPT_LOCKED_JUDGMENT = (
    "== 判断基準 ==\n"
    "1. まず predictor_scope_performance と profiles を見て、このレースでは v1-v6 のどの predictor を主参考にし、どの predictor を補助参考にするかを自分で決めること\n"
    "2. その上で predictor_horse_probs を見て、主参考 predictor と補助参考 predictor が高く評価している前5位の馬群を把握すること\n"
    "3. 量化モデルの印をそのまま写す必要はないが、主参考 predictor の前5位、および複数 predictor が強く支持する馬は本命・対抗・相手の中心候補として重点的に扱うこと\n"
    "4. horse_summary を使って、その馬群の支持の厚さとブレを確認すること\n"
    "5. 量化モデルの順位は top3 確率ベースの評価であり、1着固定の保証ではないことを前提に読むこと\n"
    "6. predictor_horse_probs と consensus_top は、前5位の中でどの馬を上位印に寄せるか、どの馬を相手へ回すかを調整する補助情報として使うこと\n"
    "7. 本命や印は前5位の中で自主的に組み替えてよく、odds・支持の厚さ・券種ごとの命中率を合わせて買い方を決めること\n"
    "8. 買い方は人間らしく、1頭だけで閉じず、通常は軸1頭と相手2-4頭程度の形を意識して組み立てること\n"
    "9. candidate_tickets の p_hit / quant_support_score / score を見て、量化前5位を軸にしつつ人間らしく命中しやすい買い方を優先すること\n"
    "10. EV は同じ方向の候補同士を並べ替える補助情報に留めること\n"
    "11. portfolio_summary は買いすぎ防止の補助情報であり、本場の主方向や主方案を変える理由にしてはいけない\n\n"
)

PROMPT_LOCKED_QUANT_RULES = (
    "== 量化モデル最優先ルール ==\n"
    "- このタスクでは量化モデルの前5位がレース方向の基準である\n"
    "- model_summary.primary_predictor は固定主軸ではなく参考情報の一つに留め、このレースでどの predictor を主参考にするかは predictor_scope_performance を見て自分で決めてよい\n"
    "- 主参考 predictor を1つ決めてもよいし、主参考1つと補助参考1つを組み合わせてもよいが、理由なく毎回 v6 固定にしてはいけない\n"
    "- まず主参考 predictor の前5位から本命候補群を作り、その後で candidate_tickets の中から買い方と金額を整えること\n"
    "- predictor_horse_probs には各モデルの全馬評価が入っているが、用途は前5位の中での強弱確認と印の並べ替えである\n"
    "- 量化モデルの順位は top3 確率ベースなので、印をそのまま写す必要はないが、前5位を軽視してはいけない\n"
    "- candidate_tickets は候補池であり、量化前5位の読みを無視して edge だけで決めてはいけない\n"
    "- marks・key_horses・focus_points は量化前5位を軸にした説明であり、主軸と逆方向の印を打たないこと\n"
    "- 量化前5位の外から主軸を作るのは、よほど強い理由がある場合を除いて避けること\n"
...
PROMPT_LOCKED_CONSTRAINTS_STATIC = (
    "== 制約 ==\n"
    "- 購入単位は100円刻み\n"
...
def get_prompt_policy_lock_info() -> Dict[str, str]:
    locked_text = "".join(
        [
            PROMPT_LOCKED_INTRO,
            PROMPT_LOCKED_ROLE,
    ...
    return {
        "version": PROMPT_POLICY_LOCK_VERSION,
        "sha256": hashlib.sha256(locked_text.encode("utf-8")).hexdigest(),
    }
...
def _make_prompt(input_obj: RacePolicyInput) -> str:
    ...
    return (
        f"{PROMPT_LOCKED_INTRO}"
        f"{PROMPT_LOCKED_ROLE}"
        f"{PROMPT_LOCKED_PUBLIC_POLICY}"
        f"{PROMPT_LOCKED_JUDGMENT}"
        f"{PROMPT_LOCKED_QUANT_RULES}"
        "== 制約 ==\n"
        ...
        f"{PROMPT_LOCKED_CONSTRAINTS_STATIC}"
        f"{PROMPT_LOCKED_OUTPUT_POLICY}"
