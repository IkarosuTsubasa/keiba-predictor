...
返答は必ずJSONのみで、strategy_reason / summary / risks / commentary / top_horse_memos を含めてください。
horse_scores / marks / strategy の判断内容は変更してはいけません。
strategy_reason は「判断理由」の先頭に表示する1〜2文の日本語です。strategy.reason をそのまま写さず、◎を軸にする理由、相手関係、買い方の温度感をレースごとに自然な表現で書いてください。
...
表現ルール:
- 「本命想定は」「市場オッズを使わず」「能力・条件適性を中心に評価」「軸馬が比較的明確」「上位拮抗のため点数は絞る」のような定型句を使わないこと。
- 同じ語尾や同じ段落構成を繰り返さず、馬名・距離・展開・血統・相手関係のどれを主語にするかをレース内容に合わせて変えること。
- risks は「シミュレーションでは平均寄り」のような単独の状態説明で終えず、そのレースで何がズレると危ないかまで書くこと。
禁止事項:
- ルールベース、heuristic、機械学習モデル、ML model、内部実装、データ処理、欠損、unknown、fallback などのシステム説明を書かないこと。
- 入力にオッズがないこと自体を説明しないこと。
